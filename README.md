# Strategic Decision Models

<div align="center">
  <a href="README.tr.md">
    <img src="https://img.shields.io/badge/Language-Türkçe-E31E24?style=for-the-badge&logo=turkey&logoColor=white" alt="Türkçe Versiyon"/>
  </a>
  <a href="README.md">
    <img src="https://img.shields.io/badge/Language-English-007EC6?style=for-the-badge&logo=us&logoColor=white" alt="English Version"/>
  </a>
</div>

<br>

This project aims to create a comprehensive series of articles on strategic decision models on GitHub. It is designed as a living knowledge base that will be updated over time with new additions.

Although I am starting without a strict roadmap, I believe the content will evolve as I write. I aim to classify and explain every decision support model I can find.

Each model will be explained in a consistent format:
* **History:** Who created it, how did it emerge?
* **Problem Definition:** Which questions does it answer?
* **Methodology:** How is it applied? (5W1H)
* **Visuals:** Diagrams and charts.
* **Use Cases:** In which sectors or scenarios is it effective?

## 📚 Table of Contents

### 1. Strategic Analysis and Situation Assessment Tools
> Tools used to understand the current situation, analyze environmental factors, and answer the question "Where are we?".

* [x] [**SWOT Analysis**](models/01-analysis/swot-analysis.md)(Strengths, Weaknesses, Opportunities, Threats)
* [x] [**PEST/PESTEL Analysis**](models/01-analysis/pestel-analysis.md) (Political, Economic, Social, Technological, Environmental, Legal)
* [x] [**Porter's 5 Forces Analysis**](models/01-analysis/porter-5-forces-analysis.md)
* [x] [**VRIO Framework**](models/01-analysis/vrio-framework.md) (Value, Rarity, Imitability, Organization) 
* [x] [**BCG Matrix**](models/01-analysis/bcg-matrix.md) (Boston Consulting Group - Growth/Share Matrix)
* [x] [**McKinsey 7S Model**](models/01-analysis/mckinsey-7s-model.md)
* [ ] **Key Success Factors (KSF) Analysis**
* [ ] **Core Competence Analysis**
* [ ] **Value Chain Analysis** (Porter)
* [ ] **Stakeholder Analysis** (Power/Interest Matrix)
* [ ] **Benchmarking**
* [ ] **Competitive Advantage Analysis**
* [ ] **Resource-Based View (RBV) Analysis**
* [ ] **Four Corners Analysis** (Porter's Competitive Strategy prediction)

---
*This repository is under active development.*